...
namespace Product_StoreDatabase1
{
    internal class ProductDatabase
    {
        ...
        public static DataSet GetTableProductGSTDetails(string Gst_Detail_Name)
        {
            SqlConnection con = GetConnection();
            string query = "\r\nselect p.cgst,p.sgst,p.igst from TableProductGSTDetails p inner join TableProductCategory t on p.Product_Gst_ID = t.Product_Gst_ID \r\nwhere Gst_Detail_Name = @Gst_Detail_Name";
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter(query, con);
