...
{
    internal class ProducctDatabase
    {
        ...
        {
            SqlConnection con = GetConnection();
            string query = "select g.CGST,g.SGST,g.IGST from TableProductGSTDetails g inner join TableProductCategory t on g.Product_Gst_ID = t.Product_Gst_ID where Product_Type_Name = @Product_Type_Name";
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter(query, con);
            da.SelectCommand.Parameters.AddWithValue("@Product_Type_Name", Product_Type_Name);
        ...
        public static DataSet GetTablProductPrice(string ProductName)
        {
            SqlConnection con = GetConnection();
